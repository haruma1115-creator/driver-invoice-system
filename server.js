...
function generateForRange(rangeStart, rangeEnd) {
  ...
  DB.offRequests.forEach(r => {
    ...
    offSet[r.date] = offSet[r.date] || {};
    offSet[r.date][r.driver] = true;
  });
  ...
  dates.forEach(date => {
    ...
    const forcedOff = drivers.filter(d => offSet[date] && offSet[date][d]);
    const pool = drivers.filter(d => !(offSet[date] && offSet[date][d]));
    ...
    const candidates = pool.filter(d => {
      const cap = capOf(d);
      return consec[d] < 6 && (weekFull[d] < cap.full || weekHalf[d] < cap.half);
    });
    candidates.sort((a, b) => (weekFull[a] + weekHalf[a] * 0.5) - (weekFull[b] + weekHalf[b] * 0.5));
    const selected = candidates.slice(0, need);
    if (selected.length < need) {
      warnings.push(`${date}(${'日月火水木金土'[dow(date)]}) 必要人数${need}名に対し出勤可能${selected.length}名`);
    }
    ...
    drivers.forEach(d => {
      ...
      if (forcedOff.includes(d)) {
        ...
        if (weekFull[d] < cap.full) { DB.schedule[date][d] = 'full'; weekFull[d]++; }
        else { DB.schedule[date][d] = 'half_am'; weekHalf[d]++; }
        consec[d]++;
      } else {
        DB.schedule[date][d] = 'off'; consec[d] = 0;
      }
...
app.post('/api/offrequests', (req, res) => {
  const { driver, dates, note } = req.body;
  if (!driver || !Array.isArray(dates) || dates.length === 0) return res.status(400).json({ error: 'invalid' });
  const created = dates.map(date => ({
    id: Date.now() + Math.random(), driver, date, note: note || '', ts: new Date().toISOString()
  }));
