...
function renderInvoiceHtml({ invoice, driver, company, salesLines, incentiveLines, expenseLines, deductionLines, otherWorkLines = [] }) {
  const isQualified = !!driver.is_qualified_invoice_issuer;

  const itemRows = [
    ...salesLines.map((l) => ({ label: '稼働費', l })),
    ...otherWorkLines.map((l) => ({ label: l.category, l })),
...
// ドライバー本人へ「支給明細が確定しました」と知らせる通知メールのHTML本文を生成する。
// 会社宛の請求書(renderInvoiceHtml)とは別の、ドライバー向けの簡易な通知文面。
function renderDriverNotificationHtml({ invoice, driver, company, loginUrl }) {
  return `<!DOCTYPE html>
<html lang="ja">
<head><meta charset="UTF-8"></head>
<body style="font-family: 'Hiragino Sans','Yu Gothic',Meiryo,sans-serif; color:#1a1a1a; max-width:560px; margin:0 auto; padding:24px; background:#fff;">
  <p>${esc(driver.name)} 様</p>
  <p>${esc(company.company_name || '')} です。<br>
  ${esc(invoice.period)}分の支給明細が確定しましたのでお知らせいたします。</p>

  <table style="width:100%; border-collapse:collapse; font-size:14px; margin:20px 0;">
    <tr><td style="padding:6px 10px; border:1px solid #ddd;">対象月</td><td style="padding:6px 10px; border:1px solid #ddd; text-align:right;">${esc(invoice.period)}</td></tr>
    <tr><td style="padding:6px 10px; border:1px solid #ddd;">請求書番号</td><td style="padding:6px 10px; border:1px solid #ddd; text-align:right;">${esc(invoice.invoice_number)}</td></tr>
    <tr><td style="padding:6px 10px; border:1px solid #ddd;">稼働費(全日+半日)</td><td style="padding:6px 10px; border:1px solid #ddd; text-align:right;">¥${yen(invoice.sales_amount)}</td></tr>
    <tr><td style="padding:6px 10px; border:1px solid #ddd;">その他稼働</td><td style="padding:6px 10px; border:1px solid #ddd; text-align:right;">¥${yen(invoice.other_work_amount)}</td></tr>
    <tr><td style="padding:6px 10px; border:1px solid #ddd;">インセンティブ+経費</td><td style="padding:6px 10px; border:1px solid #ddd; text-align:right;">¥${yen((invoice.incentives_total || 0) + (invoice.expenses_total || 0))}</td></tr>
    <tr><td style="padding:6px 10px; border:1px solid #ddd;">控除合計</td><td style="padding:6px 10px; border:1px solid #ddd; text-align:right;">−¥${yen(invoice.deductions_total)}</td></tr>
    <tr style="font-weight:bold;"><td style="padding:6px 10px; border:1px solid #ddd;">お支払見込額</td><td style="padding:6px 10px; border:1px solid #ddd; text-align:right;">¥${yen(invoice.net_payment)}</td></tr>
  </table>

  <p>詳しい内訳はマイページの「請求書一覧」からご確認いただけます。</p>
  <p style="text-align:center; margin:28px 0;">
    <a href="${esc(loginUrl)}" style="display:inline-block; background:#2b6cb0; color:#fff; text-decoration:none; padding:12px 28px; border-radius:6px; font-size:14px;">マイページを開く</a>
  </p>
  <p style="font-size:12px; color:#777;">※ このメールはシステムより自動送信されています。内容にお心当たりがない場合は、恐れ入りますが本メールを破棄してください。</p>
</body>
</html>`;
}

module.exports = { renderInvoiceHtml, renderDriverNotificationHtml };
