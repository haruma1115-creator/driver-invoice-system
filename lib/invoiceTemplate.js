...
function renderInvoiceHtml({ invoice, driver, company, salesLines, incentiveLines, expenseLines, deductionLines, nonTaxedAdditionLines = [] }) {
  ...
  const taxableRowsHtml = taxableRows.length
    ? taxableRows.map(({ label, l }) => `
      <tr>
        <td>${esc(label)}</td>
  ...
  const nonTaxedAdditionSection = nonTaxedAdditionLines.length ? `
    <div class="section-title">ガソリン代・その他稼働(消費税課税対象外・金額そのまま加算)</div>
    <table>
      <thead><tr><th>項目</th><th>内容</th><th class="num">数量</th><th class="num">単価</th><th class="num">金額</th></tr></thead>
      <tbody>
        ${nonTaxedAdditionLines.map((l) => `
          <tr>
            <td>${esc(l.category)}</td>
            <td>${l.description && l.description !== l.category ? esc(l.description) : ''}</td>
            <td class="num">${esc(l.quantity)}</td>
            <td class="num">¥${yen(l.unit_price)}</td>
            <td class="num">¥${yen(l.amount)}</td>
          </tr>
    ...
    <table class="totals">
      <tr><td>ガソリン代・その他稼働合計</td><td class="num">¥${yen(invoice.non_taxed_additions_total)}</td></tr>
    </table>` : '';
...
<body>
  ...
  <h1 class="doc-title">${isQualified ? '適格請求書' : '請求書'}</h1>
  ...
  <table class="totals">
    <tr><td>税抜合計</td><td class="num">¥${yen(invoice.taxable_base)}</td></tr>
    <tr><td>消費税(10%)</td><td class="num">¥${yen(invoice.tax_amount)}</td></tr>
    <tr><td>税込合計</td><td class="num">¥${yen(invoice.taxable_total)}</td></tr>
  </table>

  ${nonTaxedAdditionSection}
  ...
  <div class="note">
    ...
    ※ ガソリン代・その他稼働は消費税の課税対象外として、金額をそのまま合計に加算しています。<br>
    ※ 控除項目(車両代・事務手数料・車両修繕代・前借り分返済)は消費税の課税対象外の内部精算項目として、税込合計から差し引いています。実際の税務処理については税理士にご確認ください。<br>
    ${!isQualified ? '※ 発行者は適格請求書発行事業者ではないため、本書類はインボイス制度上の「適格請求書」には該当しません(区分記載請求書等保存方式の経過措置の対象となる場合があります)。<br>' : ''}
  </div>
</body>
</html>`;
}
